...
class GeminiBot(commands.Bot):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Tool mode indicator visibility per channel: channel_id -> bool
        self.channel_tool_mode_show: dict[int, bool] = {}
    ...
    def get_tool_mode_show(self, channel_id: int) -> bool:
        """Get whether to show tool mode indicator for a channel.

        Args:
            channel_id: Discord channel ID.

        Returns:
            True if indicator should be shown, False otherwise. Default is True.
        """
        return self.channel_tool_mode_show.get(channel_id, True)

    def set_tool_mode_show(self, channel_id: int, show: bool) -> None:
        """Set whether to show tool mode indicator for a channel.

        Args:
            channel_id: Discord channel ID.
            show: Whether to show the indicator.
        """
        self.channel_tool_mode_show[channel_id] = show
...
{base_prompt}
    ...
    MODEL_PRICING: dict[str, list[tuple[int | None, float, float]]] = {
        ...
        # Gemini 2.5 Flash-Lite: flat rate (must come before "gemini-2.5-flash")
        "gemini-2.5-flash": [
            (None, 0.30, 2.50),
        ],
        # --- Gemini 2.0 ---
        # Gemini 2.0 Flash: flat rate
        "gemini-2.0-flash-lite": [
            (None, 0.075, 0.30),
        ],
        # Gemini 2.0 Flash-Lite: flat rate (must come before "gemini-2.0-flash")
        "gemini-2.0-flash": [
            (None, 0.15, 0.60),
        ],
        # --- Gemini 1.5 ---
    ...
    def _format_usage_cost(self, usage_metadata: object, model: str) -> str:
        ...
        if matched_tiers is not None:
            ...
            cost_str = self.i18n.t("usage_cost_footer", tokens=token_str, total=f"{total_tokens:,}", cost=cost)
        else:
